...
export function guessLabel(path: any): string {
  ...
  const firstChild = children.find((c: any) => t.isJSXElement(c));

  if (firstChild && t.isJSXElement(firstChild)) {
    const innerName = (firstChild.openingElement.name as any)?.name;
